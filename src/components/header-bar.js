...
export class HeaderBar extends React.Component {
	...
	render() {
		...
		if (this.props.loggedIn) {
			logOutButton = <button className="header-primary-button" onClick={() => this.logOut()}>Log out</button>
		} else {
			unAuthorizedButton = (
				<div>
					<button className="header-button" onClick={() => this.signUp()}>Sign Up</button>
					<button className="header-primary-button" onClick={() => this.logIn()}>Log In</button>
				</div>
			)
		}
